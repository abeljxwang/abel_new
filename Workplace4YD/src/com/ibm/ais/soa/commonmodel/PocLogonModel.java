...
public class PocLogonModel extends ModelBase {
	...
	public ActionForward execute(Request request, Response response,
		...
		try {
			...
			if(request.getSAttrubite("user_profile")!=null){
			...
			String sql = "select FUNCID,funcname MENUNAME, func_link MENU_LINK, UP_FUNCID UP_FUNCID from "
					+ " func_tree "
					+ " where UP_FUNCID in (SELECT  A.FUNCID FUNCID "
					+ " FROM usertorole C, ROLEAUTHORIZEfunc A, func_tree B "
					+ " where  A.FUNCID = B.FUNCID AND C.ROLE_ID=A.ROLE_ID AND C.USER_ID ='"+userid+"') "
					+ " or FUNCID in (SELECT  A.FUNCID FUNCID "
					+ " FROM usertorole C, ROLEAUTHORIZEfunc A, func_tree B "
					+ "  where  A.FUNCID = B.FUNCID AND C.ROLE_ID=A.ROLE_ID AND C.USER_ID ='"+userid+"') ";
...
//			+" where  A.MENUID = B.FUNCID AND A.USER_ID = '"+userid+"'";
			...
			while (rs.next()) {
				...
				me_1.setID(rs.getString("FUNCID"));
				me_1.setUpID(rs.getString("UP_FUNCID"));
				up.addMenue(me_1);
				
				//System.out.println(me_1.getID());
